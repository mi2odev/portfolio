...
export default function Blueprint({ index, onChange }: VersionProps) {
  ...
  const secHead = (n: string, label: string, extra?: string) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: 38 }}>
      ...
      <span style={{ width: phone ? 18 : 42, height: 1.5, background: col.red, flexShrink: 0 }} />
      <span style={{ fontFamily: mono, fontSize: phone ? 11 : 12, letterSpacing: phone ? '.14em' : '.22em', textTransform: 'uppercase', color: col.ink2, padding: phone ? '0 12px' : '0 16px', whiteSpace: phone ? 'normal' : 'nowrap' }}>{label}</span>
  ...
  return (
    <div ref={rootRef} dir={dir} style={{ minHeight: '100vh', background: col.bg, color: col.ink, fontFamily: sans, position: 'relative', overflowX: 'hidden', WebkitFontSmoothing: 'antialiased' }}>
      <div data-grid style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none', backgroundImage: `linear-gradient(${col.grid} 1px,transparent 1px), linear-gradient(90deg,${col.grid} 1px,transparent 1px), linear-gradient(${col.gridM} 1px,transparent 1px), linear-gradient(90deg,${col.gridM} 1px,transparent 1px)`, backgroundSize: '28px 28px, 28px 28px, 140px 140px, 140px 140px' }} />
      ...
      <div data-coords style={{ position: 'fixed', top: 0, left: 0, zIndex: 39, pointerEvents: 'none', opacity: 0, transition: 'opacity .25s', willChange: 'transform', fontFamily: mono, fontSize: 9.5, letterSpacing: '.1em', whiteSpace: 'nowrap', padding: '4px 7px', background: 'rgba(10,39,66,0.85)', border: `1px solid ${col.line2}`, color: col.ink2 }}>
        X <span data-coord-x style={{ color: col.cyan }}>0000</span> · Y <span data-coord-y style={{ color: col.red }}>0000</span>
      </div>
      ...
      <nav style={{ position: 'fixed', top: 16, left: 16, right: 16, zIndex: 50, background: 'rgba(10,39,66,0.86)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', borderBottom: `1px solid ${col.line2}` }}>
        <div className="nav-inner" style={{ maxWidth: 1320, margin: '0 auto', padding: '12px 26px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 18 }}>
          <a href="#top" style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none', color: col.ink, flexShrink: 0 }}>
            ...
            <span style={{ fontFamily: mono, fontWeight: 500, letterSpacing: '.02em', fontSize: 13 }}>MZ<span style={{ color: col.red }}>·</span>DWG <span className="nav-tag" style={{ color: col.faint, fontSize: 11 }}>/ PORTFOLIO</span></span>
          </a>
      ...
      {phone ? (
        <div style={{ position: 'fixed', left: 16, right: 16, bottom: 16, zIndex: 45, display: 'flex', alignItems: 'center', background: 'rgba(10,39,66,0.94)', borderTop: `1px solid ${col.line2}`, fontFamily: mono, fontSize: 10, letterSpacing: '.06em', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }}>
          <span style={{ padding: '7px 10px', borderRight: `1px solid ${col.line}`, color: col.ink, whiteSpace: 'nowrap' }}>MZ-2026</span>
          <span style={{ padding: '7px 10px', borderRight: `1px solid ${col.line}`, color: col.faint, whiteSpace: 'nowrap' }}>REV <span style={{ color: col.red }}>C</span></span>
          <span style={{ padding: '7px 10px', borderRight: `1px solid ${col.line}`, color: col.faint, whiteSpace: 'nowrap' }}>SHT 06/06</span>
          <span style={{ flex: 1 }} />
          <span style={{ padding: '7px 10px', color: col.faint, whiteSpace: 'nowrap' }}>PLOT <span data-plot style={{ color: col.cyan }}>0%</span></span>
        </div>
      ) : (
      ...
      <header id="top" className="bp-hero" style={{ position: 'relative', zIndex: 2, minHeight: '100vh', display: 'flex', alignItems: 'center', padding: '140px 40px 80px', scrollMarginTop: 96 }}>
        <div data-hero style={{ maxWidth: 1320, margin: '0 auto', width: '100%', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(310px,1fr))', gap: '60px 76px', alignItems: 'center' }}>
          ...
          <div style={{ position: 'relative', justifySelf: 'center', maxWidth: 380, width: '100%', animation: 'rise .95s cubic-bezier(.2,.7,.2,1) both' }}>
            ...
            <div className="bp-dim-v" style={{ position: 'absolute', left: -38, top: 0, bottom: 0, width: 1, background: col.line2 }}>
      ...
      {/* about */}
      <section id="about" className="bp-section" style={{ position: 'relative', zIndex: 2, maxWidth: 1320, margin: '0 auto', padding: '104px 40px', scrollMarginTop: 90 }}>
      ...
      {/* skills */}
      <section id="skills" className="bp-section" style={{ position: 'relative', zIndex: 2, maxWidth: 1320, margin: '0 auto', padding: '0 40px 104px', scrollMarginTop: 90 }}>
        ...
        <div style={{ border: `1px solid ${col.line2}` }}>
          ...
          {groups.map((g) => (
            <Hover key={g.i} className="bp-skill-row" base={{ display: 'grid', gridTemplateColumns: '60px 56px 220px 1fr', borderBottom: `1px solid ${col.line}`, transition: 'background .2s' }} hover={{ background: col.panel }}>
              <span className="bp-cell" style={{ padding: '18px 14px', borderRight: `1px solid ${col.line}`, fontFamily: mono, fontSize: 12, color: col.red }}>{g.i}</span>
              <span className="bp-cell" style={{ padding: '18px 14px', borderRight: `1px solid ${col.line}`, fontFamily: mono, fontSize: 12, color: col.ink2 }}>{g.qty}</span>
              <span className="bp-cell bp-cell-last" style={{ padding: '18px 14px', borderRight: `1px solid ${col.line}`, fontFamily: sans, fontSize: 15, fontWeight: 700, display: 'flex', alignItems: 'center' }}>{g.label}</span>
              <span className="bp-cell-wide" style={{ padding: '14px 14px', display: 'flex', flexWrap: 'wrap', gap: 7, alignContent: 'center' }}>
                {g.chips.map((chip) => (
      ...
      {/* work */}
      <section id="work" className="bp-section" style={{ position: 'relative', zIndex: 2, maxWidth: 1320, margin: '0 auto', padding: '0 40px 104px', scrollMarginTop: 90 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          {sheets.map((q) => (
            <Hover key={q.no} base={{ position: 'relative', border: `1px solid ${col.line2}`, transition: 'border-color .22s' }} hover={{ borderColor: col.red }}>
              <div className="bp-sheet-head" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'stretch', borderBottom: `1px solid ${col.line2}`, fontFamily: mono }}>
                <span className="bp-sheet-cell" style={{ padding: '11px 16px', borderRight: `1px solid ${col.line2}`, fontSize: 12, color: col.red, display: 'flex', alignItems: 'center' }}>{q.no}</span>
                <span className="bp-sheet-cell" style={{ padding: '11px 16px', borderRight: `1px solid ${col.line}`, fontSize: 10, color: col.faint, letterSpacing: '.06em', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}><span style={{ opacity: 0.7 }}>DATE</span><span style={{ color: col.ink2, fontSize: 11 }}>{q.period}</span></span>
                <span className="bp-sheet-cell bp-sheet-scale" style={{ padding: '11px 16px', borderRight: `1px solid ${col.line}`, fontSize: 10, color: col.faint, letterSpacing: '.06em', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}><span style={{ opacity: 0.7 }}>SCALE</span><span style={{ color: col.ink2, fontSize: 11 }}>{q.scale}</span></span>
                <span className="bp-sheet-spacer" style={{ flex: 1 }} />
                <span className="bp-sheet-cell bp-sheet-rev" style={{ padding: '11px 16px', fontSize: 10, letterSpacing: '.08em', display: 'flex', alignItems: 'center', gap: 8, color: q.revColor }}><span style={{ width: 7, height: 7, background: q.revColor }} />{q.rev}</span>
              </div>
              <div style={{ padding: phone ? '20px 16px' : '26px 28px' }}>
      ...
      {/* education */}
      <section id="education" className="bp-section" style={{ position: 'relative', zIndex: 2, maxWidth: 1320, margin: '0 auto', padding: '0 40px 104px', scrollMarginTop: 90 }}>
        {secHead('04', b.edu.label)}
        <h2 style={{ ...h2, marginBottom: 30 }}>{b.edu.heading}</h2>
        <div style={{ border: `1px solid ${col.line2}` }}>
          <div className="skill-head" style={{ display: 'grid', gridTemplateColumns: '60px 150px 1fr', background: 'rgba(255,255,255,0.03)', borderBottom: `1px solid ${col.line2}`, fontFamily: mono, fontSize: 9.5, letterSpacing: '.1em', textTransform: 'uppercase', color: col.faint }}>
          ...
          {education.map((e, i) => (
            <Hover key={i} className="bp-edu-row" base={{ display: 'grid', gridTemplateColumns: '60px 150px 1fr', borderBottom: `1px solid ${col.line}`, transition: 'background .2s' }} hover={{ background: col.panel }}>
              <span className="bp-cell" style={{ padding: '22px 14px', borderRight: `1px solid ${col.line}`, fontFamily: mono, fontSize: 15, color: col.red, display: 'flex', alignItems: 'flex-start' }}>{e.rev}</span>
              <span className="bp-cell bp-cell-last" style={{ padding: '22px 14px', borderRight: `1px solid ${col.line}`, fontFamily: mono, fontSize: 11, color: col.ink2, display: 'flex', alignItems: 'flex-start' }}>{e.period}</span>
              <div className="bp-cell-wide" style={{ padding: '22px 18px' }}>
      ...
      {/* contact */}
      <section id="contact" className="bp-section" style={{ position: 'relative', zIndex: 2, maxWidth: 1320, margin: '0 auto', padding: '0 40px 110px', scrollMarginTop: 90 }}>
        <div style={{ position: 'relative', border: `1px solid ${col.line2}`, background: col.bg2, overflow: 'hidden' }}>
          <div style={phone
            ? { position: 'absolute', top: 14, insetInlineEnd: 14, width: 84, height: 84, border: '2px solid rgba(255,111,94,0.5)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transform: 'rotate(-13deg)', pointerEvents: 'none' }
            : { position: 'absolute', top: 24, right: 30, width: 124, height: 124, border: '2px solid rgba(255,111,94,0.5)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transform: 'rotate(-13deg)', pointerEvents: 'none' }}>
            <div style={{ position: 'absolute', inset: phone ? 5 : 7, border: '1px solid rgba(255,111,94,0.4)', borderRadius: '50%' }} />
            <span style={{ fontFamily: mono, fontSize: phone ? 9.5 : 13, letterSpacing: phone ? '.1em' : '.16em', color: col.red, textTransform: 'uppercase', textAlign: 'center', lineHeight: 1.4, padding: '0 6px' }}>{b.contact.stamp}</span>
          </div>
          <div style={{ padding: phone ? '104px 22px 40px' : 'clamp(40px,5.5vw,76px) clamp(28px,5vw,70px)' }}>
